...
def test_create_and_reply_ticket(api_client):
    ...
    assert res_create.status_code in (201, 200)
    ticket_id = res_create.json().get("id")

    # پاسخ به تیکت
    res_reply = api_client.post("/api/tickets/tickets/reply/", {"ticket": ticket_id, "message": "ممنون از پیگیری"}, format="json", **auth_header_for(u))
